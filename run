...
${CNIL}";
echo -e "${RED}| Sorry Canot install Source on Ubuntu {${Version}} Please install system To Ubuntu ${SMAY}14 or 16 ${RED} For Run The Source DANY 
${CNIL}";
exit;
fi
...
¦ NOW I Run File ${Yellow}./run${WHITB} To install ${SMAY}Source DANY${CNIL} . \n
\n\n"
chmod +x ./ins
./ins
exit;
...
if ! screen -list | grep -q "DANY"; then
screen -S DANY ./run R
fi
screen -S DANY -X kill
screen -S DANY ./run R
fi
